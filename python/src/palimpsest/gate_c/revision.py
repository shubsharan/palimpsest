...
from palimpsest.generation.text import TextSpan, apply_capitalization, render, tokenize
...
def normalized_counts(tokens: list[str]) -> Counter[str]:
...
def build_revision(
    *,
    ...
    pre_counts = normalized_counts(pre_tokens)
    post_counts = normalized_counts(post_tokens)
    eligible = sorted(
        word
...
def apply_regimes(
    ...
    *,
    stationary_key: dict[str, str],
    revised_key: dict[str, str],
    switch_word_offset: int,
) -> str:
    if switch_word_offset < 1:
        raise ValueError("Switch offset must follow at least one word token.")
    ...
    for span in tokenize(value):
        ...
        mapping = stationary_key if word_index < switch_word_offset else revised_key
        ...
        word_index += 1
    if word_index <= switch_word_offset:
        raise ValueError("Switch offset must leave at least one word token after the switch.")
