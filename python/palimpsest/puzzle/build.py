...
def _build_into(
    ...
    *,
    ...
    streams = assign_streams(plain_segments)
    vocabulary = sorted(set(_words(prepared)))
    base_key = stationary_key(vocabulary, _seed_hex(seed, "base-key"))
    eligible = eligible_symbols(streams, transition_stage, MINIMUM_STREAM_OCCURRENCES)
    ...
    contradiction = contradiction_metrics(streams, transition_stage, set(changed_symbols))
