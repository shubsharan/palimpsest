...
import pytest
from palimpsest.puzzle.package import (
    AllocationMetrics,
    AllocationSummary,
    BuildVariant,
    BuildWindow,
...
@pytest.mark.parametrize(
    ("agent_ids", "stage_count", "boundary_stage", "bytes_sha256", "content_digest"),
    [
        (
            ("alpha", "beta"),
            3,
...
def test_fixture_package_bytes_and_digests_survive_variable_geometry(
    agent_ids: tuple[str, ...],
    stage_count: int,
    boundary_stage: int,
    bytes_sha256: str,
    ...
    package_bytes = canonical_json_bytes(package.to_dict())

    decoded = FixturePackage.from_dict(package.to_dict())

    assert decoded == package
    assert decoded.content_digest == content_digest == decoded.computed_content_digest()
    ...
    assert canonical_json_bytes(decoded.to_dict()) == package_bytes
    assert hashlib.sha256(package_bytes).hexdigest() == bytes_sha256
