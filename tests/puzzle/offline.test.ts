...
import { CONDITION_IDS, RELEASE_OFFSETS_MS, resolveCondition } from "../../src/condition.js";
...
describe("offline behavior-neutral runner", () => {
  it.each(CONDITION_IDS)(
    "builds, runs, freezes, observes, and evaluates condition %s",
    async (conditionId) => {
      ...
      expect(
        releases.every((event) => {
          ...
          return typeof ordinal === "number" && event.atMs === RELEASE_OFFSETS_MS[ordinal - 1];
        }),
      ).toBe(true);
