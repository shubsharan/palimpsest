...
function validateOptions(options: AgentPromptOptions): void {
  ...
  if (options.gitVisibility === "isolated" && options.teamRoom === "enabled") {
...
function communicationText(options: AgentPromptOptions): string {
  if (options.gitVisibility === "isolated") {
...
export function buildAgentPrompt(options: AgentPromptOptions): string {
  ...
  return [
    ...
    "",
    `Additional private evidence may appear during the run. The run ends after ${String(options.cutoffMs)} milliseconds.`,
    tokenPolicy,
    "",
