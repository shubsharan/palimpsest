...
- Modified principles:
  - Environmental Constraints, Not Workflow: separates blind checking from post-freeze grading
  - Observe Outcomes Honestly: requires origin-level diagnostics and explicit team-product status
  - Condition-Defined Native Collaboration: evaluates every canonical final origin without selection
- Added sections: none
...
The runner MAY constrain what evidence is visible, when new evidence appears, which communication and checking tools exist, how much wall time or model-token usage is available, and one canonical published solver entrypoint. Agent-visible checking MUST be limited to execution, output validity, and plaintext-independent coverage. Correctness and diagnostic grading MUST occur only after model work and canonical origins are frozen. Those constraints MUST be independent of model behavior and identical for peers in the same condition. The runner MUST NOT require a particular number of model turns, Git operations, checkpoints, hypotheses, mappings, intermediate files, branches other than the published ref, or coordination sequence.
...
Incorrect solutions, early stopping, no collaboration, duplicated work, raw-text sharing, unconventional encoding, repeated checker use, source recognition, and attempts to bend the process MUST be retained as model outcomes rather than converted into invalid runs. Reports MUST separate aggregate reconstruction scores, origin-level diagnostics, realized team products, collective ceilings, observed behavior, infrastructure failures, and reviewer interpretation. Missing integration MUST remain an outcome rather than be repaired or replaced with a synthetic reconstruction. Claims MUST NOT exceed what the puzzle output and trace directly support.
...
Every assigned origin MUST begin from the same neutral solver scaffold. Only the declared pushed main solver MAY receive agent-visible blind checking. After freeze, every canonical final origin MUST receive an explicit terminal evaluation outcome: once for the shared origin in shared conditions and independently for all three private origins in isolated conditions. The runner MUST NOT select, rank, repair, merge, or substitute a preferred artifact. Git operations MUST remain unmetered, and the runner MUST NOT automate publication, merging, conflict resolution, or collaboration. Independent work, no publication, raw sharing when available, centralization, conflicts, and ignored peer work MUST remain recorded model outcomes rather than infrastructure failures.
...
- Trusted generation and grading MUST keep prepared plaintext and cipher keys unavailable to model workspaces. The blind checker MUST execute the exact solver at the assigned origin's captured `main` commit against only the caller's currently visible private evidence without opening oracle plaintext or checker truth. It MUST return only commit identity, execution and output-validity status, ciphertext and output word counts, bounded word coverage, or explicit submission errors. Post-freeze grading MAY use the oracle and MUST evaluate every condition-canonical final origin.
...
5. Tests MUST cover deterministic mechanics, agent independence, condition-defined communication visibility, identical scaffolded origins, blind published-solver checking, identical non-treatment inputs, resource cutoffs, checker non-disclosure, all-canonical-origin scoring, diagnostics, team-product semantics, and the absence of prescribed coordination workflow. Verification MUST be proportional to the claim; red-team and replay suites are not default completion requirements.
