...
- **Environmental constraints, not workflow - PASS**: Communication and evidence visibility are treatment inputs independent of model behavior. The final solver interface is fixed, but no Git operation sequence, turn, checkpoint, intermediate artifact, or coordination cadence is required.
...
`CS` and `CR` prompts are byte-identical for a given agent/config. `IS` and `IR` prompts are byte-identical. Removing the one channel paragraph makes shared and isolated prompts byte-identical. The shared paragraph encourages agents to publish, inspect, compare, and integrate through Git, while prescribing no roles, algorithms, turns, commit sequence, branch strategy, or coordination cadence. Both modes declare the same graded `origin/main:solver.py` interface.
