...
- `runs`: insertion-ordered map. Each key is the run ID; each value declares `source`, `agents`, `model`, `communication`, `releases`, `cutoff`, `spendCeilingCents`, and optional `rekeyAtStage` or `tokenLimitPerAgent`.
...
Resolution derives conventional provider credential environment names, agent IDs, uniform assignments, communication capabilities, package paths and identities, resolved milliseconds, unlimited-token nulls, and aggregate authorization. The experiment maximum is the safe-integer sum of its run ceilings.
...
One atomically published record freezes the resolved run, exact package and validation identities, releases, requested and actual model data, normalized usage, session outcomes, canonical trace paths, sandbox identity, frozen Git/workspace topology, infrastructure status, evaluation batches, and later analysis history.
