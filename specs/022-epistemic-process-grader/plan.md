...
Add a local, evidence-first grading pipeline for completed Palimpsest runs. A provider-free grading pass validates the frozen run, constructs a blinded evidence index, and computes deterministic outcome and activity measures. A separately authorized review pass deterministically compiles three ledger packets per canonical origin before provider construction, checkpoints one strict response for each applicable packet from two distinct-provider reviewers, and assembles each review without a model integration call. Protocol v4 keeps provider JSON compact: request artifacts bind packet identity, dimensions use an ordered shared-schema array with a zero-union assessment enum, and every ledger returns an episode array. Post-parse validation resolves citations and conservatively repairs episode semantics while retaining raw responses. An explicit resume command reuses only validated completed protocol-v4 packet artifacts from one named incomplete predecessor. A reporting pass links frozen process judgments to outcome and compares dimension distributions only across explicitly matched runs. The result is a non-composite scorecard that distinguishes what happened, what reviewers infer, and what the experiment can support.
...
6. Implement protocol-v4 `puzzle:review` with request-bound identities, ordered shared-schema dimension arrays, a scalar assessment enum, required episode arrays, content-addressed call artifacts, rich failure retention, conservative episode-stage and status normalization, deterministic public `ReviewerOutput` assembly, explicit same-version `--resume` lineage, and append-only completed or incomplete analyses.
...
- Reviewer tests use deterministic fake adapters to prove six ordered shared-origin calls or four isolated-origin calls, immediate packet checkpointing, compact ordered dimension arrays, empty non-epistemic episodes, conservative stage normalization, omission of revision-status episodes without revision citations, labeled assembly cautions, independent reviews, identity consistency, deterministic assembly, v4-only missing-packet resume, retained failures, preserved disagreement, and outcome-blind invariance.
