...
6. Implement protocol-v2 `puzzle:review` with fixed nullable-rating output shapes, content-addressed call artifacts, rich failure retention, conservative episode-stage normalization, deterministic `ReviewerOutput` assembly, explicit same-version `--resume` lineage, and append-only completed or incomplete analyses.
...
- Reviewer tests use deterministic fake adapters to prove six ordered shared-origin calls or four isolated-origin calls, immediate packet checkpointing, fixed dimension shapes, conservative episode-stage normalization with assembly cautions, independent reviews, identity consistency, deterministic assembly, v2-only missing-packet resume, retained failures, preserved disagreement, and outcome-blind invariance.
