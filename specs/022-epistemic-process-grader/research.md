...
**Decision**: Retain a complete local evidence index and compile one outcome-blind packet for each epistemic, social, and instrumental ledger per canonical origin. Each packet declares the evaluation unit, includes a deterministic opportunity registry, and uses non-evaluative paired/deduplicated/bounded projection. Every source item is routed or omitted explicitly; oversized reference sets retain deterministic head/tail observations and bind the ordered middle set through a content-addressed batch omission. Each serialized packet is at most 128 KiB, and provider-free preflight fails only when even the bounded reference index cannot fit.
...
Automated calibration runs on frozen synthetic contrasts and completed scorecard-v2 artifacts. It measures citation integrity, leakage resistance, explicit unit scope, stage consistency, observability distributions, and reviewer stability. It does not adjudicate reviews or establish construct validity; protocol or rubric changes receive new identities rather than rewriting historical results.
