...
**Decision**: Retain a complete local evidence index and compile one outcome-blind packet for each epistemic, social, and instrumental ledger per canonical origin. Each packet declares the evaluation unit, includes a deterministic opportunity registry, and uses non-evaluative paired/deduplicated/bounded projection. Every source item is routed or omitted explicitly. Each serialized packet is at most 128 KiB, and provider-free preflight fails if even the reference index cannot fit.
