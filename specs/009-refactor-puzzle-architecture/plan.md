...
- **Minimal reproducible mechanics — PASS**: The design removes a workspace, a barrel, an unused class, an unused parser, mixed model types, and duplicate process logic. New boundaries correspond only to current runtime responsibilities and do not add services, schemas, replay, or promotion machinery.
...
4. Atomically and exclusively persist the unchanged `attempt.json`.
5. Run overlap collection, write `overlap.json`, and append `overlap.observed`.
6. Return the unchanged success result containing `overlap`.

If attempt-summary persistence fails, observation does not start. If overlap observation fails after summary persistence, the run command remains nonzero and emits no success object; the targeted failure is appended as `overlap.failed` when the trace remains writable, the original failure remains primary, and `attempt.json`, the trace, frozen Git, and workspaces remain available to `puzzle:evaluate`.

`src/artifacts.ts` owns strict, named decoders and encoders for build results/manifests, attempt summaries, overlap results, and evaluation records. It validates schema versions, required fields, arrays, counters, agent IDs, statuses, path strings, sandbox identity, and stage cardinality. Valid stored bytes and JSON field shapes do not change; malformed records no longer flow through partial casts or silent defaults.
...
Before moving code, preserve the existing seed-17 build identity/tree digest and capture normalized seed-0 offline outputs, command key sets, checker aggregate, score, and trace partial order in `tests/golden/behavior.json`. Dynamic paths, timestamps, Git object IDs, image IDs, Docker duration, and unconstrained Git interleaving are normalized rather than frozen.
...
1. Capture stable behavior and the untracked artifact hash before moving any source.
2. Introduce root modules by moving behavior without semantic change; relocate unit tests with their owners.
3. Extract model/provider/fixture/reveal/artifact/overlap and sandbox/process boundaries; add the focused clock, decoder, scenario, and durability tests.
4. Move the Python distribution and tests in one atomic import-path/configuration change; refresh the editable environment and validate unchanged golden build bytes.
5. Switch all five scripts to `src/cli.ts`, remove the package workspace/barrel/aliases and `tools/puzzle`, then regenerate only lock metadata implied by the deleted importer.
6. Persist the attempt summary before overlap, add the targeted failure trace, and prove subsequent evaluation.
7. Update architecture, roadmap, README, formatting/lint inputs, TypeScript/Python configuration, `AGENTS.md`, `CLAUDE.md`, and current command guidance; keep `docs/proposal.md` semantically unchanged and specs 006/008 byte-for-byte unchanged.
8. Delete only revalidated generated caches/empty legacy directories, leave `.artifacts-tmp/gate-b-contract-cases.json` untouched, and execute the complete fresh acceptance sequence.
