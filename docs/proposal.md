...
**Progressive reveal, parallel and clock-driven.** If a shard arrived complete at t=0, all contradictory evidence would exist from the outset and "detection latency" would measure nothing but an agent's inspection order. Chapters are therefore released over the course of the run, each shard revealing its own chapters in parallel on a monotonic wall-time clock, identical across ablation arms and independent of agent behavior. All agents work from the start on symmetric evidence; no one is scheduled into a role. The team-level dynamic that results is reconciliation under ambiguity: agents on opposite sides of a switch build locally consistent but mutually contradictory dictionaries and must *discover through the repository* that the disagreement is structured by book position rather than by error. The within-agent anchor-then-revise dynamic, where the cleanest measurement lives, is untouched — an agent whose shard interior contains a switch still builds a mapping from its early chapters and collides with its own later ones. "Sufficiently contradictory," for latency purposes, is defined by the oracle at generation time — a threshold on contradicting token mass released — not after observing agent behavior.
...
Communication makes the attribution possible but not automatic. What resolves it is a *pattern*: several mappings failing together, failures localized by book position, matched unchanged controls still holding, later-shard evidence supporting the revised entries. The channel's job is to expose that spatial structure, which no agent can see alone, to let the team recognize that a disagreement is organized by position in the book rather than by which teammate messed up. To carry that structure cheaply, committed dictionary entries include lightweight provenance: supporting chapter range, local occurrence count, confidence, and the trusted publication slot of the latest validation event. Provenance is exactly the kind of compressed evidence the budget is designed to favor over raw relay.
...
**The run.** No rounds, no commit order. Agents run concurrently and pull, compute, and push at will, matching the actual shape of asynchronous collaborative work. The scheduler supplies neither mutual exclusion nor a global turn clock, so failures to pull, duplicated work, stale pushes, and merge conflicts remain part of the coordination trace. Fairness across runs and models is enforced on compute: fixed per-agent budgets for model tokens, invocations, CPU, and wall time. A push deadline freezes the repository; a short final window follows in which agents may pull the frozen state before producing their private deliverables. Each agent computes privately with unrestricted local tools and may submit any network-disabled executable that satisfies the solver contract. No network is available beyond authenticated Git. Final deliverables go to a private, unbudgeted, per-agent output directory invisible to teammates. Reconstructions cannot live in the repository, since a mostly solved reconstruction *is* the shard and committing it would launder the budget.
...
The baseline suite uses a compact five-rung ladder that also describes the expected solution path:

1. frequency-rank assignment with POS and syntactic-context typing
...
5. the strongest preceding decoder with oracle segmentation

Target-identification and retrieval attacks form a separate mandatory baseline track. The interpretive frame follows from the ladder's dual role. Because every decoding rung is something the agent could also build, an agent's score is meaningful *relative to the highest rung it effectively reproduced*. A frontier agent scoring near rung 3 has re-derived known technique; an agent scoring well above rung 4 has done something the non-oracle ladder cannot account for; an agent scoring below rung 2 has failed at execution despite plausibly knowing the method — and that last outcome, which the long-horizon literature predicts is likely, is the most interesting result the puzzle can produce. We aren't saying the scoring headroom is attributable to semantics, but that the ladder estimates how much headroom remains beyond increasingly expressive statistical attacks.
...
The final rung isolates whether switch detection creates useful difficulty or merely breaks every solver indiscriminately. The design is viable only if measurable headroom remains.
...
Residual risk is real. Recognition-level memory of an obscure book still supplies plot priors, though name regeneration weakens them. Fresh private text — post-cutoff generated fiction or licensed unpublished prose — is used when identification attacks exceed the accepted threshold for public-domain sources.
...
Two mechanisms secure the channel, and they must not be conflated. **Logical Git-state metering bounds the main channel.** A byte cap alone is not an information bound because bits can ride in filenames, commit messages, author fields, timestamps, branch names, object selection, graph topology, and push patterns. Agents therefore use authenticated standard Git through a receive gateway that charges each accepted transaction with a deterministic `GitAccountingFrameV1`. The frame includes the raw ref operation and every logical commit, tree, and blob becoming peer-visible for the first time. Rejected pushes are not charged or exposed. Unreachable objects, unsafe refs, force pushes, deletion, tags, notes, replace refs, LFS, symlinks, submodules, alternates, and push options are rejected. Fetch packs are regenerated so sender-selected pack compression and delta choices are not visible to peers. Fixed publication slots and rate limits bound timing and push-presence channels separately. **The budget sweep addresses capacity.** The design requires a budget interval in which agents can transmit a useful evolving belief state — dictionaries with provenance, switch hypotheses, solver diffs — but cannot transmit a complete shard under the strongest tested compressor, granting the adversary all shared side information and summing over the whole run. The nominal separation between key information and compressed shard size makes this interval plausible but not assured; the exact Git accounting implementation determines whether it exists.
...
**Switch-detection gaming.** Chapter boundaries are structurally visible, so an agent can flag every boundary and score well under a tolerance-window metric. The scoring policy publishes *S*, requires exactly *S* predictions, and uses event-level precision and recall with penalties for premature alarms.
...
| --------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| Compressed shard relay            | Adversarial compressor suite using exact Git accounting; budget set below tested relay threshold    | Capacity margin may prove too thin, forcing geometry redesign                                    |
| Repository metadata side channels | Meter raw logical ref, commit, tree, and blob bytes in `GitAccountingFrameV1`                        | Meter omissions can reopen a channel                                                             |
| Pack and storage representation    | Isolate unpublished objects and regenerate fetch packs from immutable snapshots                     | Git implementation changes require renewed audit                                                 |
| Push timing and presence           | Fixed publication slots plus attempt and pull rate limits                                            | Slot choice and contention retain bounded capacity                                               |
| Static-evidence latency artifact  | Parallel progressive reveal on a fixed clock                                                        | Reveal cadence interacts with agent speed; clock and budgets held constant across arms           |
...
**Switch detection** by event-level precision and recall with premature-alarm penalties. Detection latency is measured on the monotonic wall-time reveal clock, from the oracle-defined release of sufficiently contradictory text to the first correct anomaly or regime-fork publication; adaptation latency runs from detection until changed-entry accuracy crosses a preset threshold. The headline revision plot shows four trajectories per switch: changed-entry accuracy, unchanged-entry accuracy, false retractions of stable entries, and recovery of newly changed ones — failed stay and failed update, operationalized without pretending to a general belief-revision benchmark.

**Collaboration**, measured by a matched ablation. The primary comparison is **communication-enabled three-agent versus communication-disabled three-agent**, identical in shard assignment, reveal clock, model-call and token allowance, local compute, deadlines, and seeds, over paired repetitions — enough that one lucky team run cannot carry the conclusion, at calibration scale rather than publication-grade confidence intervals. Deterministic grading does not make the treatment effect deterministic. We employ a secondary concept called **team scale-up gain**: it bundles extra compute, distributed observation, ensembling, and communication, and is reported as exactly that bundle. For the public artifact, one compelling run can still be the showpiece; the paired ablation is the engineering evidence behind it.

**Calibration** (optional) by Brier score over per-entry confidences.

Because no exact attainable ceiling is computable, scores are reported raw and against the best ladder rung, with a small expert-human or human-plus-tools baseline as an empirical reference.

Alongside the numbers, the accepted Git history and trusted event log ship as first-class artifacts: the epistemic cascade — observation, interpretation, commit, publication, pull, revision — ordered by server events, with stale consensus, confident errors becoming committed truth, and anchoring on prior-regime mappings all directly inspectable.
...
One relevant comparison is GBAEval, which under the same standard has no ablation matrix, no construct-validity analysis, no confidence intervals, and no human calibration panel, and which explicitly declines to present itself as a complete measure of software-engineering ability. Palimpsest adopts the same limitation language, with one asymmetry conceded rather than contested: GBAEval's oracle directly measures the artifact it names, while Palimpsest's oracle directly measures restored text and recovered mappings — not semantic reasoning, collaboration, or belief revision as traits. Those appear throughout this document as intended demands, observable behaviors, and diagnostics, validated as load-bearing by feasibility and matched calibration comparisons — never as separately certified measurements. Those comparisons exist because a few are needed to interpret the score at all, not as a claim to have decomposed the construct.

A public evaluation would require a family of controlled nonstationary decipherment environments under matched ablations. Palimpsest itself remains one compound puzzle.
...
**Asynchronous harness.** A TypeScript/Node control plane owns run lifecycle, reveal timing, quotas, container supervision, Git admission, publication slots, ledgers, event logging, freeze, and the operator CLI. Per-agent containers expose only authenticated standard Git. The Git Gateway meters accepted peer-visible logical state, rejects unsafe Git features, and publishes immutable snapshots on fixed slots. The reveal daemon releases chapters on the monotonic wall clock. Compute logs cover model input and output tokens, invocation count, CPU, memory, wall time, context policy, file-read policy, subagent policy, and the exact scaffold.

**Grader and replay.** A separately versioned Python package owns generation, baselines, solver re-execution, scorers, replay analysis, metrics, and plots. Replay uses frozen artifacts, accepted Git/ref/ledger history, event logs, and private outputs; it does not attempt to reproduce agent scheduling or decisions.

**Matched calibration.** The minimum set needed to interpret a score includes stationary versus changing keys, communication-enabled versus communication-disabled teams, oracle segmentation versus hidden switches, and one centralized agent with all shards as the pooling upper bound. Books, seeds, shard-to-agent assignments, and repetitions are paired; matched mean differences are calibration evidence rather than publication statistics.

**Red team.** The security suite covers conditional compression, Git logical and transport channels, timing, source identification, isolation, grader gaming, hostile solver bundles, and degenerate strategies.
...
7. shard and agent count
8. byte budget
9. run duration and per-agent compute budget
...
## Design Parameters and Risks

**Text corpus tiers.** Tier 1 uses the Gutenberg long tail with proper-noun regeneration, structural deduplication, and cipher-view canary screening, preserving both automatic authoring and the "corpus is the key" structure. Tier 2 uses recently *digitized* public-domain works whose first digital transcription postdates the evaluated model's training window. This reduces contamination without licensing exposure or a register mismatch against the public-domain reference corpus. Tier 3 uses fresh private text, generated post-cutoff or licensed unpublished prose. Tier 1 is rejected when identification attacks exceed the accepted threshold after regeneration or plot-level recall measurably lifts scores.

**Ablation scope.** The four core conditions are stationary versus changing keys, communication-enabled versus communication-disabled teams, hidden versus oracle segmentation, and centralized all-shard solving. A full factorial, human panels, or transfer-instance generalization would define a broader evaluation family rather than this single puzzle.

**NER quality risk.** Proper-noun regeneration on 19th-century prose can miss entities, mangle titles, and over-capture common nouns. Feasibility evidence measures missed entities, over-capture, and cross-mention consistency.

**Budget calibration.** The byte budget must create selection pressure inside a verified interval: useful evolving beliefs fit, while the best measured complete-shard relay does not. Failure on either side requires changing the information geometry rather than weakening the accounting rule.

**Asynchrony variance.** Concurrent scheduling, polling, and contention raise run-to-run variance, which paired repetitions absorb at calibration scale. Fixed publication slots and rate limits bound push-cadence channels. Agents construct their own coordination; the scheduler supplies neither mutual exclusion nor a turn clock.

**Determinism.** Grading and artifact replay are deterministic for sealed inputs and pinned environments. Agent behavior and operating-system scheduling remain stochastic.

**Citation hygiene.** Every external claim requires a verifiable primary source. Recent arXiv identifiers require verification before publication.
