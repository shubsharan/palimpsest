...
**The run.** No rounds, no commit order. Agents run concurrently and pull, compute, and push at will, matching the actual shape of asynchronous collaborative work. This removes two synchronization gifts that turn-taking silently provides — free mutual exclusion and a free global clock — so that agents which fail to pull before committing, duplicate one another's work, or clobber entries produce exactly the inter-agent coordination failures the trace exists to observe, rather than having them designed away by a scheduler. Fairness across runs and models is enforced where it belongs, on compute rather than turns: fixed per-agent budgets for model tokens, invocations, and wall time. A push deadline freezes the repository; a short final window follows in which agents may pull the frozen state before producing their private deliverables. Throughout, each agent pulls, computes privately against its shard with unrestricted local Python, and pushes within budget. No network beyond the repository; executed code may not call external model APIs. Final deliverables go to a private, unbudgeted, per-agent output directory invisible to teammates. Reconstructions cannot live in the repository, since a mostly-solved reconstruction *is* the shard and committing it would launder the budget.
...
Two distinct mechanisms secure the channel, and they must not be conflated. **Canonicalization closes side channels.** A byte cap alone is not an information bound — bits ride in filenames, commit messages, author fields, timestamps, branch and tag names, object-ID selection, push counts, and rejection patterns — so the raw pre-receive hook is replaced by a canonical message object: an append-only per-agent message log at a fixed path on a fixed branch, metadata normalized and timestamped server-side, each push re-serialized canonically before counting against the ledger, with a server-enforced minimum interval between pushes so that push cadence itself cannot become an unmetered timing channel. Git remains the visible interface and the audit trail; it is not the security boundary. **The budget sweep addresses capacity.**Canonicalization does nothing to stop an optimally compressed shard riding inside the legitimate payload, so the design stands on an explicit feasibility criterion: **there must exist a budget interval in which agents can transmit a useful evolving belief state — dictionaries with provenance, switch hypotheses, solver diffs — but cannot transmit a complete shard under the strongest tested compressor**, granting the adversary all shared side information (known vocabulary ordering, the reference corpus, common codebooks, custom arithmetic coders) and summing over the whole run. The ~3× nominal separation between key information and compressed shard size makes this interval plausible but not assured; testing it is the first go/no-go check in Phase 0, before any container infrastructure exists.
...
| Repository metadata side channels | Server-normalized append-only message log; minimum push interval                                    | Infrastructure-level timing channels still need audit                                            |
...
**Phase 2 — Harness.** Per-agent containers, network namespace exposing only the repository. Canonical message object with server-side normalization, cumulative byte ledger, and push rate-limiting. A reveal daemon releasing chapters on the fixed clock; fully asynchronous agent execution under per-agent compute budgets (tokens, invocations, wall time); push deadline and final output window. Explicit compute-boundary specification and logging: total input and output tokens, model invocation count, CPU time, memory, context-truncation policy, file-read limits, whether subagents are permitted, and the exact scaffold — an agent's work is bounded by inference budget and persistent workspace, not by one context window, and unlogged this is uninterpretable.

**Phase 3 — Grader and replay.** All scorers; solver re-execution for reproducibility; replay from seed, commit log, and private outputs; automatic time-series trajectory plots for changed and unchanged entries.

**Phase 4 — Matched calibration runs.** The minimum set needed to interpret a score, at calibration scale: stationary versus changing key; the comm/no-comm treatment from gate D, now paired and repeated; oracle segmentation versus hidden switches; one centralized agent with all shards (the pooling upper bound). A handful of books, seeds, shard-to-agent assignments, and paired repetitions; matched mean differences, honestly labeled as calibration rather than publication statistics.
...
**Asynchrony costs, accepted deliberately.** Removing turn structure raises run-to-run variance (scheduling, polling behavior, contention), which the paired repetitions absorb at calibration scale, and opens push cadence as a potential timing channel, bounded by the server-side minimum push interval. The trade is judged worthwhile because turn-taking silently supplies mutual exclusion and a global clock — synchronization that real collaborators must construct, and whose absence is where coordination failures become observable.
...
**Citation hygiene.** A conformity-cascade claim considered during research has been dropped pending a verifiable source. All arXiv identifiers below should be re-verified before this document goes external; several are recent and were reported second-hand during research.
